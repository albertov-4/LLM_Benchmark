...
If LLM planning is closer to approximate retrieval than to search (Kambhampati 2024; Valmeekam 2023), then 

(a) retries are near-independent samples so success must be discounted by retry count this justifies the creation of \emph{FASR, IWSR}

(b) feedback content is largely irrelevant so a large retry gap is not correction \emph{Stechly 2023}

(c) the only way to separate retrieval from reasoning is to probe execution, grounding and reasoning-faithfulness directly and this justifies the creation of \emph{exec\_ratio, PAS, hallucination, CoTaL}
