...
\textbf{Failure anatomy --- ER + PAS}(most useful when \(\mathrm{SR}<0.15\): how deep does it run, and why did it stopc)
...
\addlinespace
\(\mathrm{IHR}>0.90\) \(\cdot\) \(\mathrm{PAS}>0.70\) & top-left (good) & Knows vocabulary, ordering errors only --- most fixable.\\
\addlinespace
\(\mathrm{IHR}>0.90\) \(\cdot\) \(\mathrm{PAS}<0.30\) & bottom-left & Knows words, lacks world model.\\
\addlinespace
\(\mathrm{IHR}<0.60\) \(\cdot\) \(\mathrm{PAS}\) low & bottom-right (worst) & Near-random output --- no grounding, no causal model.\\
\bottomrule
\end{tabularx}
\end{table}
