...
\begin{description}
\item[chart] per-model scatter grid, faceted by domain/task
\item[scope] for each model, exec\_ratio against plan lenght, with one subplot per domain/task and points colored by difficulty tier.
\item[how to read] within one model, compare domain/task subplots: a downward trend means executability decays on longer plans. A trend present in every subplot suggests a general lenght/state-tracking limit, while a trend isolated to one subplot suggests a domain/task-specific weakness.
...
\begin{description}
\item[chart] one heatmap per domain.
\item[scope] Per-domain normalised rank for every RANK\_METRIC. Each cell is a rank-derived value, not the raw metric value: \(1=\) best model for that metric within that domain, \(0=\) worst.
\item[how to read] Read each domain file independently. Compare cells down a column to see which model leads on that metric within that domain. Do not read the annotated number as SR, FASR, Exec, PAS, etc.; it is the normalised rank position.
\item[data problems] Normalisation is per-domain and rank-based. Equal colours across different domain heatmaps mean equal relative position, not equal raw performance.
