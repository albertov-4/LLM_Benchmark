...
\begin{bmplotcard}{Hallucination heatmap}
\code{hallucination\_heatmap}
\begin{description}
\item[chart] heatmap
\item[scope] Mean action-hallucination rate per model\(\times\)domain. Lower = better grounded.
...
\item[data problems] A model with SR\(\approx\)0 can still show 0 hallucination if it never emitted a parseable action --- read alongside Exec.
\item[why this form] Heatmap is right because the data is a dense model\(\times\)domain grid of one bounded scalar; small multiples would waste space.
\end{description}
\end{bmplotcard}
...
\begin{bmplotcard}{FASR vs IWSR scatter}
...
\begin{bmplotcard}{Success rate by CoT flag}
...
\begin{bmplotcard}{CoT alignment: valid vs invalid}
...
\begin{bmplotcard}{Failure-mode taxonomy}
...
\item[how to read] Higher = domain specialist (phi\_4 \(\approx\)2.39); lower = generalist (gpt\_oss \(\approx\)1.35).
...
The 27 figures are run-specific --- they depend on which models and domains were in the bundle and would be stale the moment the suite is re-run. This catalogue documents what each figure

\emph{means and how to read it}
