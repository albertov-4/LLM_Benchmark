...
These metrics should be read as a cascade: executability is the first gate, PAS becomes meaningful only when the plan runs far enough for the simulator to observe state transitions.\\
For near-solving models very small failure counts make sequencing/fabrication proportions unstable, so the breakdown must be read with SR and FASR.