...
\item[In the plots] \code{success\_rate\_heatmap}(model\(\times\)domain), the SR bars in \code{sr\_vs\_fasr} and \code{sr\_fasr\_iwsr\_by\_model}, and it drives the colour in \code{failure\_mode\_taxonomy} and \code{fasr\_iwsr\_scatter}.
...
FASR=("_iter1", "mean")
\end{lstlisting}
\item[Range \& meaningful bands] FASR>0.50 with low Retry Gap \(\rightarrow\) genuine zero-shot capability. FASR \(\approx\) 0 while SR is high \(\rightarrow\) the model only ever succeeds by resampling (the entire qwen / nemotron pattern in the bundled run).
...
\end{lstlisting}
\item[Range \& meaningful bands] RG<0.10 \(\rightarrow\) near-zero-shot. 0.10--0.30 \(\rightarrow\) Efficient Corrector territory.>0.30 \(\rightarrow\) Stochastic Searcher: success is mostly resampling. In the bundled run qwen\_3\_5 hits 0.500.
...
\addlinespace
large & small & FASR \(\approx\) IWSR \(\ll\) SR --- most success arrives very late (attempt 4--5), heavily penalised by IWSR. & Stochastic / late luck\\
\addlinespace
