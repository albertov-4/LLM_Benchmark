...
\texttt{raw/} stores the original model generations, \texttt{parsed/} stores the extracted action sequences, and \texttt{scored/} stores the validation outcomes produced after VAL has checked the candidate plan. The analysis script joins these layers with the selected PDDL task files, so every row can be evaluated against the exact domain and problem that generated it.
...
\texttt{df\_metrics} is the central object of the analysis. It starts from the artifact rows and is horizontally extended with computed columns. Its grain is one benchmark instance, not one retry attempt. This is important because success rates, hallucination rates and executability scores are averaged over comparable units.
...
The simulator starts from the problem initial state, applies each known action step by step, and stops at the first unsatisfied precondition. If a missing proposition was true before, the error is classified as sequencing; if it was never true, it is classified as state fabrication. VAL remains the official correctness oracle; the simulator is a diagnostic layer for explaining invalid plans.