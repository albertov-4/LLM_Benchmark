...
The pipeline emits up to 27 figures, registered by name in \code{PLOT\_DESCRIPTIONS} and written by \code{maybe\_make\_plots}. This catalogue documents each one along the four axes that decide whether a figure earns its place: its\textbf{scope}(what slice of data it shows),\textbf{how to read it}, the\textbf{data-availability problems} that distort it on sparse runs, and the\textbf{justification} for that chart type over the alternatives.

\begin{quote}\small
\textbf{One reading rule that governs half the figures.} Several plots are bounded [0,1] and zero-inflated: on weak models the bars genuinely collapse to zero, and a zero can mean either "good" (no hallucination) or "nothing produced" (no actions emitted). Whenever a bar is flat-zero, disambiguate against Success\_Rate and plan counts before reading it as a strength. The atlas notes this per-plot where it bites.
\end{quote}

\subsubsection{Grounding / discriminatory}

\paragraph{Hallucination heatmap.}
\code{hallucination\_heatmap}
\begin{description}
\item[chart] heatmap
\item[scope] Mean action-hallucination rate per model--domain. Lower = better grounded.
\item[how to read] Read down a column to compare models in one domain; dark = invents illegal actions. Empty cells = no plans produced there.
\item[data problems] A model with SR--0 can still show 0 hallucination if it never emitted a parseable action -- read alongside Exec.
\item[why this form] Heatmap is right because the data is a dense model--domain grid of one bounded scalar; small multiples would waste space.
\end{description}

\paragraph{Action hallucination (bars).}
...
\item[scope] Distribution of exec\_ratio (valid-prefix length / plan length) per model--domain.
...
\paragraph{Executability vs plan length.}
...
\paragraph{Failure type breakdown.}
...
\paragraph{SR vs FASR.}
\code{sr\_vs\_fasr}
\begin{description}
\item[chart] paired bars
\item[scope] Overall success rate beside first-attempt success rate, per model.
\item[how to read] The vertical gap between the pair is the Retry Gap; a large gap = retry-dependent success.
\item[data problems] Both can be 0 for weak models -- the pair collapses and the gap is trivially zero.
\item[why this form] Pairing the two bars makes the gap a visual quantity instead of a number to subtract mentally.
\end{description}

\paragraph{FASR by difficulty.}
...
\paragraph{IWSR by model--domain.}
\code{iwsr\_by\_model\_domain}
\begin{description}
\item[chart] grouped bars
\item[scope] Iteration-weighted success rate (1/iterations on success, else 0) per model--domain.
...
\paragraph{SR -- FASR -- IWSR triptych.}
\code{sr\_fasr\_iwsr\_by\_model}
\begin{description}
\item[chart] grouped bars
\item[scope] All three success metrics side by side per model.
\item[how to read] Read the descent SR--IWSR--FASR: a steep drop = retry-inflated; a flat triple = genuine first-try skill.
\item[data problems] For SR--0 models all three vanish together, conveying little beyond'failed'.
...
\paragraph{FASR vs IWSR scatter.}
\code{fasr\_iwsr\_scatter}
\begin{description}
\item[chart] bubble scatter
\item[scope] FASR (x) vs IWSR (y) with SR encoded as dot size and colour.
\item[how to read] On-diagonal = first-try-dominated; below-diagonal with large dot = success built by efficient repair.
\item[data problems] Overplotting at the origin when many models score \textasciitilde{}0; jitter or label to separate.
\item[why this form] Encoding three metrics (x,y,size/colour) in one scatter is denser than three separate bar charts.
\end{description}

\paragraph{P(Valid | reached k).}
...
\paragraph{CoT alignment.}
\code{cot\_alignment\_by\_model\_domain}
\begin{description}
\item[chart] grouped bars
\item[scope] Mean CoTaL (reasoning-vs-plan alignment) per model--domain.
...
\paragraph{Success rate by CoT flag.}
\code{cot\_success\_rate}
\begin{description}
\item[chart] paired bars
\item[scope] Success rate split by whether CoT was present/enabled.
\item[how to read] A lift with CoT suggests reasoning helps; no lift suggests post-hoc rationalisation.
\item[data problems] Confounded if CoT presence correlates with model family rather than being randomised.
\item[why this form] A two-bar split is the minimal correct contrast for a binary flag.
\end{description}

\paragraph{CoT alignment: valid vs invalid.}
\code{cot\_alignment\_validity}
\begin{description}
\item[chart] box / violin
\item[scope] Distribution of CoTaL for valid versus invalid plans.
\item[how to read] Higher alignment on valid plans = faithful reasoning; overlap = alignment doesn't track correctness.
\item[data problems] Needs enough valid AND invalid reasoning plans; the bundled run has few of either (18 both-valid, 8 both-invalid).
\item[why this form] A distribution contrast is needed because the claim is about separation, not means.
\end{description}

\subsubsection{Composite\&cross-domain}

\paragraph{Success-rate heatmap.}
\code{success\_rate\_heatmap}
\begin{description}
\item[chart] heatmap
\item[scope] Raw SR per model--domain -- no normalisation, so cells are comparable across the whole grid.
...
\code{failure\_mode\_taxonomy}
\begin{description}
\item[chart] quadrant bubble
\item[scope] Halluc (x) vs PAS (y), bubble size = FASR, colour = SR -- four named failure regimes.
\item[how to read] Locate each model in a quadrant; same-SR models in different quadrants fail differently. (See Cross-Domain view.)
\item[data problems] Bubbles collapse near an axis when a metric is 0/NaN for many models; small samples make positions unstable.
\item[why this form] Two orthogonal failure axes plus two encodings packs the entire failure diagnosis into one figure.
\end{description}

\paragraph{Composite Planning Score.}
...
\paragraph{Within-domain rank heatmap.}
...
\item[how to read] Higher = domain specialist (phi\_4 --2.39); lower = generalist (gpt\_oss --1.35).
...
\paragraph{Domain correlation (Spearman).}
\code{domain\_correlation}
\begin{description}
\item[chart] heatmap
\item[scope] Model--model Spearman -- of per-domain success-rate vectors.
\item[how to read] --1 = redundant probes; low/negative -- = complementary models worth keeping both.
\item[data problems] Needs several domains with variance; with few domains and many SR=0 ties -- is dominated by ties.
\item[why this form] A correlation matrix is the canonical view for pairwise similarity across a model set.
\end{description}

\paragraph{PS stacked by domain.}
...
\paragraph{Metrics summary table.}
...
\paragraph{Capability radar.}
\code{radar\_chart}
\begin{description}
\item[chart] radar
\item[scope] Polar pentagon of FASR -- IWSR -- Exec -- IHR -- PAS per model (base profile, no CoT bonus).
