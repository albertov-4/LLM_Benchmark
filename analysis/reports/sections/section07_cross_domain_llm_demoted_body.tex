...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.80\linewidth]{../../results/final_evaluation_results/p_valid_given_k.png}
    \caption{Probability of a valid plan at iteration k}
    \label{fig:pvalidk}
\end{figure}

By looking at the graph \texttt{Kimi\_k2.6} has an initial behaviour common with the other LLMs but due to its dataset the downward part does not provide informastions.\\
\texttt{Nemotron Nano}, \texttt{Nemotron Ultra} \texttt{GPT oss} exhibit a quasi-flat behaviour, this would suggest that these LLMs are doing re-sampling instead of corrections but \texttt{GPT oss} and \texttt{Nemotron Ultra} have an inital upward slope so a correction behaviour could be expected.\\
About \texttt{PHI\_4\_ mini} it is stale dataset so we cannot take out anything. \texttt{Qwen}'s behaviout would suggest some correction capabilities but due to the rippling behaviour we can see that the output could be very stochastic oriented.\\
\vspace{0.2cm}
\textbf{Note:} All these considerations are made based on the type of feedback we provide.
